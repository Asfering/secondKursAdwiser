...
def AddUser(login, psw):
    ...
    try:
        ...
        mainCursor.execute(
            "INSERT INTO Users(UserID, UserLogin, UserPassword, IsAdmin) VALUES ((SELECT COUNT(Users.UserID) from Users)+1, ?, ?, 0);",
            (login, psw))
...
def CreateStudent(user_id):
    ...
    if current_user.is_admin(mainCursor):  # если пользователь админ
        if request.method == "POST":
            if len(request.form['fstName']) > 1 and len(request.form['scdName']) > 2 and request.form[
                'userId'] is not None and request.form['groupId'] is not None:  # простая проверка
                res = AddStudent(request.form['fstName'], request.form['scdName'], request.form['thrName'],
                                 request.form['userId'], request.form['groupId'])  # создаем студента
                if res:
                    flash("Вы успешно добавили студента", "success")  # Сообщение пользователю
                    return redirect(
                        url_for('CreateStudent', user_id=current_user.get_id()))  # переход на эту же страницу
                else:
                    flash("Ошибка при добавлении в БД", "error")  # Сообщение пользователю
            else:
                flash("Неверно заполнены поля", "error")  # Сообщение пользователю
...
def AddStudent(name, lastName, middleName, userId, groupId):
    ...
    try:
        ...
        mainCursor.execute("INSERT INTO Students(StudentID, UserID, StudentFirstName, StudentLastName, StudentMiddleName, GroupID) \
            VALUES ((SELECT COUNT(Students.StudentID) from Students)+1, ?, ?, ?, ?, ?)",
                           (userId, name, lastName, middleName, groupId))
        ...
        listC = ["'StudentID'", "'UserID'", "'StudentFirstName'", "'StudentLastName'", "'StudentMiddleName'",
                 "'GroupID'"]
...
def CreateProfessor(user_id):
    ...
    if current_user.is_admin(mainCursor):  # если пользователь админ
        if request.method == "POST":
            if len(request.form['fstName']) > 1 and len(request.form['scdName']) > 2 and request.form[
                'userId'] is not None:  # простая проверка
                res = AddProfessor(request.form['fstName'], request.form['scdName'], request.form['thrName'],
                                   request.form['userId'])  # создание преподавателя
                if res:
                    flash("Вы успешно добавили преподавателя", "success")  # Сообщение пользователю
                    return redirect(
                        url_for('CreateProfessor', user_id=current_user.get_id()))  # переход на эту же страницу
                else:
                    flash("Ошибка при добавлении в БД", "error")  # Сообщение пользователю
            else:
                flash("Неверно заполнены поля", "error")  # Сообщение пользователю
...
def AddProfessor(name, lastName, middleName, userId):
    ...
    try:
        ...
        mainCursor.execute("INSERT INTO Professors(ProfessorID, UserID, ProfessorFirstName, ProfessorLastName, ProfessorMiddleName) \
            VALUES ((SELECT COUNT(Professors.ProfessorID) from Professors)+1, ?, ?, ?, ?)",
                           (userId, name, lastName, middleName))
    ...
    if request.method == "POST":
        ...
        if user and psw[0] == request.form['psw']:  # если совпадают пароль и логин
            userlogin = UserLogin().create(
                user)  # авторизация пользователя, запоминаем ID текущего пользователя для дальнейшей работы
...
def createWork(user_id):
    ...
    if current_user.is_professor(mainCursor):  # если пользователь преподаватель
        if request.method == "POST":
            if len(request.form['nameOfWork']) > 5 and len(request.form['Task']) > 10 and request.form[
                'numberOfGroup'] is not None and request.form['day'] is not None \
                    and request.form['month'] is not None and request.form['year'] is not None and request.form['hour']\
                    is not None and request.form['minute']:  # простая проверка
                res = AddWork(request.form['nameOfWork'], request.form['Task'], request.form['numberOfGroup'],
                              request.form['year'], request.form['month'], request.form['day'], request.form['hour'],
                              request.form['minute'])  # создание записи в бд
                if res:
...
def AddWork(nameOfWork, Task, numberOfGroup, year, month, day, hour, minute):
    ...
    try:
        ...
        listC = ["'WorkID'", "'ProfessorID'", "'Task'", "'DateOfBegin'", "'DateOfEnd'", "'NameOfWork'", "'UrlOfWork'",
                 "'GroupID'"]
        listD = [f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'",
                 f"'{strName}'", f"'{strName}'"]
        listE = ['Null', 'Null', 'Null', 'Null', 'Null', 'Null', 'Null', 'Null']
        listF = [count[0], f'{current_user.get_id()}', f"'{Task}'", 'GETDATE()',
                 f'DATETIMEFROMPARTS({year}, {month}, {day}, {hour}, {minute}, 00, 000)', f"'{nameOfWork}'",
                 f"'{stringNameWork}'", f"'{numberOfGroup}'"]
        listOfLists = [listA, listB, listC, listD, listE, listF]
...
def profile(user_id):
    ...
    if current_user.is_student(mainCursor):  # Если студент
        ...
        if current_user.is_admin(mainCursor):  # Если админ. !!!Важно - Админ имеет роль студента!!!
            role = "Администратор"
            return render_template("profileadm.html", title=f"Профиль - {nameOfUser}", nameOfAdmin=nameOfUser,
                                   role=role)
...
def showTask(user_id, id_post):
    ...
    if current_user.is_student(mainCursor):  # если студент
        ...
        flash("У вас нет прав доступа к данной странице", "error")  # Сообщение пользователю
        return redirect(url_for('login'))
    return render_template("task.html", title=title, task=task, DateOfBeginHTML=DateOfBegin, DateOfEndHTML=DateOfEnd,
                           professorHTML=professor, tf=tf, post=id_post)
...
def CreateEssayResponce(user_id, post_id, stdResponce):
    ...
    try:
        ...
        listC = ["'EssayID'", "'WorkID'", "'UserID'", "'StudentEssay'", "'ResultEssay'", "'Grade'", "'Comment'",
                 "'DateOfUpload'"]
        listD = [f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'", f"'{strName}'",
                 f"'{strName}'", f"'{strName}'"]
        listE = ['Null', 'Null', 'Null', 'Null', 'Null', 'Null', 'Null', 'Null']
        listF = [count[0], f"'{post_id}'", f"'{user_id}'", f"'{stdResponce}'", f"'{stdResponce}2'", f"'{gradeRandom}'",
                 'NULL', 'GETDATE()']
        listOfLists = [listA, listB, listC, listD, listE, listF]
        Recording(listOfLists, 8)
    except:
...
def showWorksByMe(user_id):
    ...
    if not current_user.is_professor(mainCursor):  # если пользователь не препод
        flash("У вас нет прав доступа к данной странице", "error")  # Сообщение пользователю
        return redirect(url_for('login'))
    return render_template("wrkbyme.html", title="Выданные мной задания",
                           tasksBeforeDeadline=fDataBase.getTaskAnnonceByMe(mainCursor,
                                                                            current_user.id_of_professor(mainCursor)), \
                           tasksAfterDeadline=fDataBase.getTaskAfterDeadline(mainCursor,
                                                                             current_user.id_of_professor(mainCursor)))
...
def essayResp(post_id, user_id):
    ...
    if current_user.is_professor(mainCursor):  # если пользователь препод
        ...
        flash("У вас нет прав доступа к данной странице", "error")  # Сообщение пользователю
        return redirect(url_for('login'))
    return render_template("essayresponse.html", title=f"Ответы к заданию {nameOfWork}", nameOfWork=nameOfWork,
                           task=taskOfWork,
                           responses=fDataBase.getEssayResponse(current_user.id_of_professor(mainCursor), mainCursor,
                                                                post_id))
    ...
    if current_user.is_professor(mainCursor):  # если пользователь препод
        ...
        if request.method == "POST":
            if len(request.form['comment']) > 2:  # процесс добавление комментария. минимум 2 знака.
                ...
                UpdateComment(request.form['comment'], post_id, comOldEssay,
                              student_id)  # изменяем комментарий к работе
